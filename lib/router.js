...
Router.configure({
  ...
  waitOn: function() { return [Meteor.subscribe('films'),Meteor.subscribe('votes')] }
});
...
Router.route('/video/:_id', function () {
  ...
  var myFile = Files.findOne({title: this.params._id});
  console.log(myFile)
  var path = Meteor.settings.public.videoFolder + 'files-' + myFile._id + '-' + myFile.original.name;
  console.log(path);
...
Router.route('/films/:_id', {
  ...
  waitOn: function() {
    ...
    return [Meteor.subscribe('medias', this.params._id),Meteor.subscribe('responses')];
  },
